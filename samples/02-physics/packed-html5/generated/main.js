...
InputSystem.prototype = $extend(ash_core_System.prototype,{
	...
	,update: function(dt) {
		...
		var mouseScreenPosition = Module.Vector2(mousePosition.x / 800,mousePosition.y / 600);
		var mouseWorldPosition = this.camera.screenToWorldPoint(Module.Vector3(mouseScreenPosition.x,mouseScreenPosition.y,0));
		...
		if(input.getMouseButtonPress(4)) {
			...
			var this1 = Module.Vector2(mouseWorldPosition.x,mouseWorldPosition.y);
			var this2 = Module.Vector2(mouseWorldPosition.x,mouseWorldPosition.y - 20000);
			tmp.raycastSingle(result,this1,this2);
...
Application.start = function(engine) {
	...
	e.set_position(Module.Vector3(0,-400,0));
	engine.addEntity(e);
	gengine.getRenderer().getDefaultZone().setFogColor(Module.Color(0.5,0.5,0.5,1));
	engine.getSystem(FactorySystem).spawnCrate(64,Module.Vector3(0,300,0));
	var result = new Module.PhysicsRaycastResult2D();
	var tmp6 = sceneEntity.get(gengine_components_PhysicsWorld2D);
	var this6 = Module.Vector2(0,0);
	var this7 = Module.Vector2(0,-2000);
	tmp6.raycastSingle(result,this6,this7);
};
...
ash_core_ComponentMatchingFamily.prototype = {
	...
	,addIfMatch: function(entity) {
		if(this.entities.h.__keys__[entity.__id__] == null) {
			...
			while(componentClass.hasNext()) if(!entity.has(componentClass.next())) {
				return;
			}
			var node = this.nodePool.get();
			node.entity = entity;
			var _this1 = this.components.keyMap;
			var componentClass1 = new haxe_ds__$StringMap_StringMapIterator(_this1,_this1.arrayKeys());
			while(componentClass1.hasNext()) {
				var componentClass2 = componentClass1.next();
				var _this2 = this.components.valueMap;
				var key = Type.getClassName(componentClass2);
				node[__map_reserved[key] != null?_this2.getReserved(key):_this2.h[key]] = entity.get(componentClass2);
			}
...
ash_core_Engine.prototype = {
	addEntity: function(entity) {
		...
		while(family.hasNext()) family.next().newEntity(entity);
	...
	,removeEntity: function(entity) {
		...
		while(family.hasNext()) family.next().removeEntity(entity);
	...
	,componentAdded: function(entity,componentClass) {
		var _this = this.families.valueMap;
		var family = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(family.hasNext()) family.next().componentAddedToEntity(entity,componentClass);
	}
	,componentRemoved: function(entity,componentClass) {
		var _this = this.families.valueMap;
		var family = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(family.hasNext()) family.next().componentRemovedFromEntity(entity,componentClass);
	}
...
ash_core_Entity.prototype = {
	...
	,getAll: function() {
		...
		while(component.hasNext()) componentArray.push(component.next());
...
ash_core_NodePool.prototype = {
	...
	,dispose: function(node) {
		...
		while(componentName.hasNext()) node[componentName.next()] = null;
		node.entity = null;
		node.next = null;
		node.previous = this.tail;
		this.tail = node;
