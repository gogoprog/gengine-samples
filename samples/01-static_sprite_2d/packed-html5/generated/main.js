...
var MainSystem = function() {
	this.time = 0.0;
	ash_core_System.call(this);
};
MainSystem.__name__ = ["MainSystem"];
MainSystem.__super__ = ash_core_System;
MainSystem.prototype = $extend(ash_core_System.prototype,{
	addToEngine: function(engine) {
		this.logoEntity = new gengine_Entity();
		this.logoEntity.add(new gengine_components_StaticSprite2D());
		var staticSprite2D = this.logoEntity.get(gengine_components_StaticSprite2D);
		var sprite = gengine.getResourceCache().getSprite2D("logo.png",true);
		staticSprite2D.object.setSprite(sprite);
		engine.addEntity(this.logoEntity);
	}
	,update: function(dt) {
		this.time += dt;
		var position = this.logoEntity.get_position();
		position.x = Math.cos(this.time * 2) * 128;
		this.logoEntity.set_position(position);
		if(gengine.getInput().getScancodePress(41)) {
			gengine.exit();
		}
	}
	,__class__: MainSystem
});
...
Application.start = function(engine) {
	...
	var this1 = Module.Color(0.5,0.5,0.5,1);
