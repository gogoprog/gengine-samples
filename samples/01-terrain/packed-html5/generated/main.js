...
GameSystem.prototype = $extend(ash_core_System.prototype,{
	addToEngine: function(engine) {
		this.terrainEntity = new gengine_Entity();
		this.terrainEntity.add(new gengine_components_Terrain());
		this.terrainEntity.get(gengine_components_Terrain).setPatchSize(64);
		var tmp = this.terrainEntity.get(gengine_components_Terrain);
		var this1 = Module.Vector3(1.0,0.5,1.0);
		tmp.setSpacing(this1);
		this.terrainEntity.get(gengine_components_Terrain).setSmoothing(true);
		this.terrainEntity.get(gengine_components_Terrain).setCastShadows(true);
		var tmp1 = this.terrainEntity.get(gengine_components_Terrain);
		var tmp2 = gengine.getResourceCache().getImage("HeightMap.png",true);
		tmp1.setHeightMap(tmp2);
		var tmp3 = this.terrainEntity.get(gengine_components_Terrain);
		var tmp4 = gengine.getResourceCache().getMaterial("Terrain.xml",true);
		tmp3.setMaterial(tmp4);
		var tmp5 = this.terrainEntity;
		var this2 = Module.Vector3(-0.7,0,-0.2);
		tmp5.setPosition(this2);
		engine.addEntity(this.terrainEntity);
		this.cameraEntity = new gengine_Entity();
		this.cameraEntity.add(new gengine_components_Camera());
		engine.addEntity(this.cameraEntity);
		var tmp6 = this.cameraEntity;
		var this3 = Module.Vector3(0.0,30.0,-30.0);
		tmp6.setPosition(this3);
		var _this = this.cameraEntity;
		var y = this.terrainEntity.get(gengine_components_Terrain);
		var this4 = Module.Vector3(0,0,0);
		var this5 = Module.Vector3(0,y.getHeight(this4) + 15.0,0);
		var upVector = null;
		if(upVector == null) {
			var this6 = Module.Vector3(0,1,0);
			upVector = this6;
		}
		_this.node.lookAt(this5,upVector,2);
		var viewport = new Module.Viewport(gengine.getContext());
		viewport.setScene(gengine.getScene());
		viewport.setCamera(this.cameraEntity.get(gengine_components_Camera).object);
		...
		tmp9.setMaterial1(tmp10);
		engine.addEntity(skyEntity);
	}
	,update: function(dt) {
		this.angle += dt * 0.5;
		var position = this.cameraEntity.get_position();
		position.x = 100 * Math.cos(this.angle);
		position.z = 100 * Math.sin(this.angle);
		...
		_this.node.lookAt(this1,upVector,2);
