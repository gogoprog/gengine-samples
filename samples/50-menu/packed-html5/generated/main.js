...
GameSystem.prototype = $extend(ash_core_System.prototype,{
	addToEngine: function(engine) {
		...
		this.modelEntity.get(gengine_components_AnimatedModel).setModel(gengine.getResourceCache().getModel("Ninja.mdl",true));
		this.modelEntity.get(gengine_components_AnimatedModel).setMaterial(gengine.getResourceCache().getMaterial("Ninja.xml",true));
		this.modelEntity.setPosition(Module.Vector3(-0.7,0,-0.2));
	...
	,update: function(dt) {
		...
		this.modelEntity.get(gengine_components_AnimatedModel).getAnimationStateByIndex(0).addTime(dt);
...
Application.start = function(engine) {
	...
	cameraEntity.setPosition(Module.Vector3(0.0,2.0,-3.0));
	var position = Module.Vector3(0,1,0);
	var upVector = null;
	if(upVector == null) {
		upVector = Module.Vector3(0,1,0);
	}
	cameraEntity.node.lookAt(position,upVector,2);
	...
	lightEntity.setPosition(Module.Vector3(0.0,5.0,-5.0));
	var this1 = Module.Vector3(0.0,-1.0,1.0);
	lightEntity.node.setDirection(this1);
...
ash_core_ComponentMatchingFamily.prototype = {
	...
	,addIfMatch: function(entity) {
		if(this.entities.h.__keys__[entity.__id__] == null) {
			...
			while(tmp.hasNext()) if(!entity.has(tmp.next())) {
				return;
			}
			...
			while(tmp1.hasNext()) {
				var componentClass = tmp1.next();
				var _this2 = this.components.valueMap;
				var key = Type.getClassName(componentClass);
				node[__map_reserved[key] != null?_this2.getReserved(key):_this2.h[key]] = entity.get(componentClass);
			}
...
ash_core_Engine.prototype = {
	addEntity: function(entity) {
		...
		while(tmp.hasNext()) tmp.next().newEntity(entity);
	...
	,removeEntity: function(entity) {
		...
		var tmp = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(tmp.hasNext()) tmp.next().removeEntity(entity);
	...
	,componentAdded: function(entity,componentClass) {
		var _this = this.families.valueMap;
		var tmp = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(tmp.hasNext()) tmp.next().componentAddedToEntity(entity,componentClass);
	}
	,componentRemoved: function(entity,componentClass) {
		var _this = this.families.valueMap;
		var tmp = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(tmp.hasNext()) tmp.next().componentRemovedFromEntity(entity,componentClass);
	}
...
ash_core_Entity.prototype = {
	...
	,getAll: function() {
		...
		var _this = this.components.valueMap;
		var tmp = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(tmp.hasNext()) componentArray.push(tmp.next());
...
ash_core_NodePool.prototype = {
	...
	,dispose: function(node) {
		var _this = this.components.valueMap;
		var tmp = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
		while(tmp.hasNext()) node[tmp.next()] = null;
		node.entity = null;
		node.next = null;
		node.previous = this.tail;
		this.tail = node;
...
gengine_components_AnimatedModel.__super__ = gengine_components_UrhoComponent;
gengine_components_AnimatedModel.prototype = $extend(gengine_components_UrhoComponent.prototype,{
	setModel: function(model,createBones) {
		if(createBones == null) {
			createBones = true;
		}
	...
	,getAnimationStateByIndex: function(index) {
...
gengine_components_Light.__super__ = gengine_components_UrhoComponent;
gengine_components_Light.prototype = $extend(gengine_components_UrhoComponent.prototype,{
