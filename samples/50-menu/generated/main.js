...
GameSystem.prototype = $extend(ash_core_System.prototype,{
	update: function(dt) {
		if(gengineApp.getInput().getScancodePress(41)) gengineApp.exit();
	}
...
Application.init = function() {
	gengineApp.setWindowSize(Module.IntVector2(1024,768));
	gengineApp.setWindowTitle("50-menu");
	gengineApp.setGuiFilename("gui/gui.html");
};
...
var gengine_Entity = function() {
	ash_core_Entity.call(this);
	this.node = new Module.Node(gengineApp.getContext());;
};
...
gengine_Entity.prototype = $extend(ash_core_Entity.prototype,{
	add: function(component,componentClass) {
		if(js_Boot.__instanceof(component,gengine_components_UrhoComponent)) {
			this.node.addComponent(component.object, 0, 0);
		}
	...
	,remove: function(componentClass) {
		...
		if(component != null && js_Boot.__instanceof(component,gengine_components_UrhoComponent)) {
			this.node.removeComponent(component.object);
		}
		return component;
	}
...
gengine_Main.start = function() {
	...
	window.dummyNode = gengineApp.getScene().createChild(0, 0);;
